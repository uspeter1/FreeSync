...
// The vault-list dashboard content, without any chrome. Used both by the
// standalone /dashboard route (wrapped in AppShell) and by the marketing
// shell as inline tab content when the user is signed in.
//
// Renders: header row (title + Join by code / + New vault buttons), inline
// forms for create/join, then two conditional sections — Pending
// invitations (with Accept/Decline) and Your vaults (with per-vault cards
// linking to /dashboard/vaults/:id).
...
const styles: Record<string, React.CSSProperties> = {
  ...
  primaryBtn: { padding: '8px 14px', background: THEME.accent, color: '#fff', border: 'none', borderRadius: 6, fontSize: 13, fontWeight: 500, cursor: 'pointer' },
  secondaryBtn: { padding: '8px 14px', background: THEME.surface, color: THEME.text, border: `1px solid ${THEME.border}`, borderRadius: 6, fontSize: 13, cursor: 'pointer' },
  linkBtn: { padding: '8px 6px', background: 'transparent', color: THEME.textMuted, border: 'none', fontSize: 13, cursor: 'pointer' },
  inlineForm: { display: 'flex', gap: 8, alignItems: 'center', padding: 14, background: THEME.surface, border: `1px solid ${THEME.border}`, borderRadius: 8, marginBottom: 24 },
  input: { flex: 1, padding: '8px 10px', background: THEME.bg, color: THEME.textBright, border: `1px solid ${THEME.border}`, borderRadius: 6, fontSize: 13, outline: 'none' },
  ...
  empty: { padding: '40px 24px', background: THEME.surface, border: `1px solid ${THEME.border}`, borderRadius: 10, textAlign: 'center' },
  list: { listStyle: 'none' },
  card: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 18px', background: THEME.surface, border: `1px solid ${THEME.border}`, borderRadius: 8, marginBottom: 10, textDecoration: 'none', color: THEME.text, transition: 'background 0.12s, border-color 0.12s' },
  cardTitle: { fontSize: 15, fontWeight: 500, color: THEME.textBright },
  cardMeta: { display: 'flex', alignItems: 'center', gap: 10, fontSize: 12, color: THEME.textMuted },
  badge: { padding: '2px 8px', background: THEME.accentSoft, color: THEME.link, borderRadius: 10, fontSize: 11, fontWeight: 500 },
  sectionHeading: { fontSize: 12, fontWeight: 600, color: THEME.textMuted, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 10 },
  invitedCard: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 18px', background: THEME.surface, border: `1px solid ${THEME.link}`, borderRadius: 8, marginBottom: 10, gap: 16 },
  invitedBadge: { padding: '2px 8px', background: 'rgba(167,139,250,0.14)', color: THEME.link, borderRadius: 10, fontSize: 11, fontWeight: 500 },
};
